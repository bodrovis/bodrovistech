...
#### Почему generator должен перейти в generator
...
где `G` — cyclic group порядка `4`.

Любой элемент `Z4` получается из `1`:
...
Поэтому после выбора значения `φ(1)` весь mapping уже определяется автоматически:

```text
φ(k) = φ(1)^k
```

В частности, все значения mapping лежат в подгруппе:
...
Теперь предположим, что `φ` должна быть isomorphism. Тогда она обязана быть onto:
...
А это как раз означает, что `φ(1)` должен быть generator группы `G`.

#### Что будет, если отправить `1` не в generator
...
Потому что степени каждого из них проходят по всем четырём элементам.
...
Поэтому mapping сохраняет операцию и является homomorphism, но не является isomorphism: он не one-to-one и не onto.
...
Это тоже generator, поэтому получится другой, но тоже правильный isomorphism.

#### Возвращаемся к операциям

Теперь проверим операцию. В `Z4`:
...
#### Откуда берётся формула `φ(k) = 3^k mod 10`

Мы строим isomorphism:

```text
φ : Z4 -> U(10)
```

Группа `Z4` порождается элементом `1` под сложением:

```text
Z4 = <1>
```

А группа `U(10)` порождается элементом `3` под умножением modulo `10`:

```text
U(10) = <3>
```

Чтобы mapping был onto, generator `1` должен перейти в generator целевой группы. Поэтому задаём:

```text
φ(1) = 3
```

Теперь остальные значения уже определяются требованием сохранить операцию.
...
Identity тоже обязана перейти в identity:
...
```text
φ(k mod 4) = 3^k mod 10
```

Формула не выбрана случайно:

* `k` означает сложить generator `1` с собой `k` раз;
* isomorphism переводит это сложение в умножение;
* поэтому результатом становится произведение `k` копий числа `3`, то есть `3^k`;
* поскольку целевая группа — `U(10)`, результат вычисляется modulo `10`.
...
#### Возвращаемся к конкретному примеру
