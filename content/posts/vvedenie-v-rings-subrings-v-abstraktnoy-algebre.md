...
Но в нашем учебнике ring **не обязано** иметь multiplicative identity. Поэтому мы будем различать ring и ring with unity.
...
Поэтому нельзя путать `0` и `1`. Первый связан со сложением, второй — с умножением.
...
Теперь введём ещё один важный термин: unit.

> **Unit / обратимый элемент кольца** — это элемент ring, который имеет multiplicative inverse внутри этого же ring. Это элемент, который можно умножить на какой-то другой элемент и получить `1`.

То есть элемент `a` является unit, если существует элемент `a^-1` такой, что:

```text
aa^-1 = a^-1a = 1
```

Элемент `a^-1` называется multiplicative inverse элемента `a`.

#### Почему важно “внутри того же ring”

В real numbers элемент `2` обратим:

```text
2 · 1/2 = 1
```

Поэтому `2` является unit в `R`.

Но в integers `1/2` не лежит:
...
Один и тот же элемент может быть unit в одном ring и не быть unit в другом.
...
операция multiplication выполняется modulo `10`.

Элемент `3` является unit, потому что:
...
Элемент `7` тоже является unit, потому что:

```text
7 · 3 = 21 ≡ 1 (mod 10)
```

А вот элемент `2` не является unit в `Z10`.
...
Мы ни разу не получили `1`. Значит нет такого элемента `b ∈ Z10`, что:
...
```text
2 is not a unit in Z10
```

#### Units в `Z_n`

В ring `Z_n` units — это элементы, которые имеют multiplicative inverse modulo `n`. Именно они образуют группу:
...
Это все элементы `Z10`, которые обратимы по multiplication modulo `10`.

Есть удобный критерий:

```text
a is a unit in Z_n iff gcd(a, n) = 1
```
...
Других units нет. Например, для `2` потребовался бы элемент:
...
не является integer. Но он принадлежит:
...
Functions здесь складываются и умножаются **pointwise / поточечно**. Это означает, что мы берём конкретное значение `x` и выполняем операцию над значениями функций в этой точке.
...
По addition ring всегда является Abelian group. Multiplication обязано быть associative и distributive относительно addition.
...
Из closure under subtraction ничего не следует про multiplication. Поэтому нужно дополнительно проверить:
...
Следовательно, `4` играет роль unity внутри `S`. Получается:
...
Это не противоречие. Unity определяется относительно конкретного ring и его элементов. Элемент `4` не является identity для всего `Z6`, например:
...
элемент `1` не лежит в `nZ`. И другого multiplicative identity там тоже нет.
...
Другой функции, способной быть multiplicative identity для всех функций из `S`, тоже нет. Значит `S` — ring without unity.
