---
title: "Group Homomorphisms: отображения, kernel и потеря информации"
draft: false
date: 2026-06-15
meta_desc: "Понятное введение в group homomorphisms: отличие от isomorphisms, kernel, image, свойства, cosets kernel и числовые примеры. Теорема homomorphism."
tags:
- абстрактная алгебра
- cs
- zero knowledge
---

В главе про isomorphisms мы рассматривали отображения, которые показывают, что две группы имеют одну и ту же структуру.

Isomorphism должен:

* сохранять group operation;
* быть one-to-one;
* быть onto.

Но часто нам нужно отображение, которое сохраняет операцию, даже если часть информации при этом теряется. Такое отображение называется **group homomorphism / гомоморфизмом групп**.

{{< video 3yhWN5QVVkE >}}

{{< video ZNIlnGrZBqQ >}}

## Главная идея

Homomorphism переводит элементы одной группы в другую так, чтобы выполнение операции до и после mapping давало одинаковый результат.

Если:

```text
φ : G -> H
```

— homomorphism, то:

```text
φ(ab) = φ(a)φ(b)
```

для любых:

```text
a, b ∈ G
```

Слева мы:

1. сначала выполняем операцию в `G`;
2. затем применяем `φ`.

Справа мы:

1. сначала применяем `φ` к обоим элементам;
2. затем выполняем операцию в `H`.

Результат должен совпасть.

### В additive notation

Если обе группы записаны аддитивно, условие принимает вид:

```text
φ(a + b) = φ(a) + φ(b)
```

Например:

```text
φ : Z -> Z_n
```

```text
φ(m) = m mod n
```

Тогда:

```text
φ(a + b) = (a + b) mod n
```

и:

```text
φ(a) + φ(b)
=
(a mod n) + (b mod n)
```

После reduction modulo `n` результаты совпадают.

### Homomorphism и isomorphism — не одно и то же

> Каждый isomorphism является homomorphism. Но не каждый homomorphism является isomorphism.

Homomorphism обязан сохранять операцию, однако он может:

* отправлять разные элементы в один и тот же результат;
* не попадать во все элементы codomain;
* уменьшать порядки элементов;
* сжимать большую группу в меньшую.

То есть homomorphism может терять информацию.

## Пример: integers переходят в `Z_n`

Рассмотрим mapping:

```text
φ : Z -> Z_n
```

```text
φ(m) = m mod n
```

Например, при:

```text
n = 5
```

получаем:

```text
φ(2) = 2
φ(7) = 2
φ(12) = 2
φ(-3) = 2
```

Разные целые числа переходят в один элемент `Z5`. Значит mapping не one-to-one.

Но операция сохраняется:

```text
φ(a + b) = φ(a) + φ(b)
```

Например:

```text
a = 7
b = 9
```

Сначала складываем в `Z`:

```text
7 + 9 = 16
```

Применяем mapping:

```text
φ(16) = 1
```

Теперь другим путём:

```text
φ(7) = 2
φ(9) = 4
```

Складываем modulo `5`:

```text
2 + 4 = 6 ≡ 1 (mod 5)
```

Получили тот же результат:

```text
φ(7 + 9) = φ(7) + φ(9)
```

Следовательно, это homomorphism.

## Kernel: какие элементы превращаются в identity

Одна из главных вещей, связанных с homomorphism, — его **kernel / ядро**.

Пусть:

```text
φ : G -> H
```

Тогда kernel определяется как:

```text
Ker φ = {g ∈ G | φ(g) = e_H}
```

Здесь `e_H` — identity element целевой группы `H`.

> Kernel содержит все элементы исходной группы, которые mapping превращает в identity.

### Интуитивный смысл kernel

Kernel показывает:

> какую часть исходной группы homomorphism перестаёт различать.

Все элементы kernel становятся identity в целевой группе.

Если kernel содержит только identity:

```text
Ker φ = {e_G}
```

то mapping ничего лишнего не склеивает и является one-to-one. Если kernel содержит несколько элементов, mapping теряет информацию.

### Kernel mapping `Z -> Z_n`

Вернёмся к:

```text
φ : Z -> Z_n
```

```text
φ(m) = m mod n
```

Identity в additive group `Z_n` — это `0`.

Поэтому kernel состоит из всех целых чисел, которые переходят в `0 modulo n`:

```text
Ker φ = {..., -2n, -n, 0, n, 2n, ...}
```

То есть:

```text
Ker φ = nZ
```

Например, при `n = 5`:

```text
Ker φ = 5Z
```

```text
Ker φ = {..., -10, -5, 0, 5, 10, ...}
```

Именно числа, отличающиеся на элемент kernel, переходят в одинаковый результат:

```text
2
7
12
-3
```

Все они отличаются друг от друга на числа, кратные `5`.

## Image: что осталось после mapping

Кроме kernel, важно понятие **image / образа**.

Image homomorphism:

```text
φ(G)
```

— это множество всех элементов целевой группы, в которые mapping действительно попадает:

```text
φ(G) = {φ(g) | g ∈ G}
```

Codomain может быть больше image.

Например:

```text
φ : R* -> R*
```

```text
φ(x) = x^2
```

Здесь `R*` — nonzero real numbers под умножением.

Любой результат `x²` положительный, поэтому:

```text
φ(R*) = R_positive
```

Mapping не попадает в отрицательные числа.

Следовательно, он не onto как mapping:

```text
R* -> R*
```

### Пример: `φ(x) = x²` под умножением

Рассмотрим multiplicative group:

```text
R* = R \ {0}
```

То есть `R*` состоит из всех ненулевых real numbers, а групповой операцией является multiplication. `\ {0}` — значит "без нуля".

Определим mapping:

```text
φ : R* -> R*
```

по правилу:

```text
φ(x) = x²
```

Mapping действительно переводит элементы `R*` обратно в `R*`: если `x != 0`, то:

```text
x² != 0
```

#### Проверяем сохранение операции

Чтобы `φ` была homomorphism, должно выполняться:

```text
φ(ab) = φ(a)φ(b)
```

для любых:

```text
a, b ∈ R*
```

Считаем левую сторону:

```text
φ(ab) = (ab)²
```

Так как multiplication real numbers коммутативно:

```text
(ab)² = abab = aabb = a²b²
```

А:

```text
a² = φ(a)
b² = φ(b)
```

Следовательно:

```text
φ(ab) = a²b² = φ(a)φ(b)
```

Значит:

```text
φ(x) = x²
```

является group homomorphism из `(R*, ·)` в `(R*, ·)`.

#### Kernel

Kernel состоит из элементов исходной группы, которые переходят в identity целевой группы.

Identity в multiplicative group — это `1`. Поэтому ищем все `x ∈ R*`, для которых:

```text
φ(x) = 1
```

То есть:

```text
x² = 1
```

У этого уравнения два решения:

```text
x = 1
x = -1
```

Следовательно:

```text
Ker φ = {1, -1}
```

#### Что означает этот kernel

Mapping не различает числа, отличающиеся только знаком:

```text
φ(x) = x²
```

и:

```text
φ(-x) = (-x)² = x²
```

Поэтому:

```text
φ(x) = φ(-x)
```

Например:

```text
φ(3) = 9
φ(-3) = 9
```

```text
φ(5) = 25
φ(-5) = 25
```

Именно элементы kernel:

```text
{1, -1}
```

объясняют это склеивание: `x` и `-x` отличаются умножением на `-1`, а `-1` лежит в kernel.

#### Image

Квадрат любого ненулевого real number положителен, поэтому:

```text
φ(R*) = R_positive
```

Mapping не попадает в отрицательные числа.

Следовательно, как mapping:

```text
φ : R* -> R*
```

он не является onto.

Он также не является one-to-one, потому что:

```text
φ(x) = φ(-x)
```

Но он является `2-to-1` mapping на свой image:

```text
R_positive
```

Каждое положительное число `y` имеет два preimages:

```text
sqrt(y)
```

и:

```text
-sqrt(y)
```

## Non-example: `φ(x) = x²` под сложением

Теперь рассмотрим ту же формулу, но другую group operation:

```text
φ : (R, +) -> (R, +)
```

```text
φ(x) = x^2
```

Для homomorphism должно выполняться:

```text
φ(a + b) = φ(a) + φ(b)
```

Но:

```text
φ(a + b) = (a + b)^2
```

```text
φ(a) + φ(b) = a^2 + b^2
```

generally:

```text
(a + b)^2 != a^2 + b^2
```

Например:

```text
a = 2
b = 3
```

Тогда:

```text
φ(2 + 3) = φ(5) = 25
```

Но:

```text
φ(2) + φ(3) = 4 + 9 = 13
```

Поэтому:

```text
25 != 13
```

Следовательно, mapping не является homomorphism группы `(R, +)`.

Одна и та же функция может быть homomorphism для одной group operation и не быть homomorphism для другой.

## Пример: absolute value

Рассмотрим nonzero real numbers под умножением `R*` и mapping:

```text
φ(x) = |x|
```

Проверяем операцию:

```text
φ(xy) = |xy|
```

Но:

```text
|xy| = |x||y|
```

Следовательно:

```text
φ(xy) = φ(x)φ(y)
```

Значит absolute value — homomorphism.

Kernel:

```text
Ker φ = {x ∈ R* | |x| = 1}
```

Поэтому:

```text
Ker φ = {1, -1}
```

Mapping "забывает" знак числа:

```text
2 и -2 -> 2
5 и -5 -> 5
```

## Homomorphism обязан быть well-defined

Когда элементы исходной группы могут иметь несколько записей, нужно проверить, что mapping не зависит от выбранной записи. Это называется **well-defined / корректно определённое отображение**.

Рассмотрим quotient group:

```text
Z / 3Z
```

В ней:

```text
0 + 3Z = 3 + 3Z
```

потому что `0` и `3` лежат в одном coset.

Допустим, мы пытаемся определить:

```text
φ(x + 3Z) = 3x mod 6
```

Проверим два representatives одного coset.

Для `x = 0`:

```text
φ(0 + 3Z) = 3 · 0 = 0 mod 6
```

Для `x = 3`:

```text
φ(3 + 3Z) = 3 · 3 = 9 ≡ 3 (mod 6)
```

Но:

```text
0 + 3Z = 3 + 3Z
```

а результаты разные:

```text
0 != 3 in Z6
```

Значит это вообще не функция на cosets. Она не well-defined.

Главная проверка:

> разные representatives одного элемента должны давать одинаковый результат.

## Основные свойства homomorphisms

Пусть:

```text
φ : G -> H
```

— group homomorphism.

Тогда автоматически выполняется несколько важных свойств.

### 1. Identity переходит в identity

```text
φ(e_G) = e_H
```

Почему?

```text
φ(e_G)
=
φ(e_Ge_G)
=
φ(e_G)φ(e_G)
```

Сокращаем один множитель:

```text
φ(e_G) = e_H
```

### 2. Степени сохраняются

Для любого integer `n`:

```text
φ(g^n) = φ(g)^n
```

Например:

```text
φ(g^3)
=
φ(ggg)
=
φ(g)φ(g)φ(g)
=
φ(g)^3
```

В additive notation:

```text
φ(ng) = nφ(g)
```

### 3. Порядок образа делит порядок исходного элемента

Если:

```text
|g| = n
```

то:

```text
|φ(g)| divides n
```

Потому что:

```text
g^n = e_G
```

Применяем `φ`:

```text
φ(g^n) = φ(e_G)
```

Следовательно:

```text
φ(g)^n = e_H
```

Значит порядок `φ(g)` делит `n`.

Homomorphism может уменьшить порядок элемента, но не может сделать его больше.

#### Пример

Пусть элемент `g` имеет порядок `6`. Тогда порядок `φ(g)` может быть:

```text
1, 2, 3 или 6
```

Но не может быть:

```text
4, 5, 7, ...
```

### 4. Kernel является подгруппой

```text
Ker φ ≤ G
```

Действительно, если:

```text
a, b ∈ Ker φ
```

то:

```text
φ(a) = e_H
φ(b) = e_H
```

Следовательно:

```text
φ(ab^-1)
=
φ(a)φ(b)^-1
=
e_He_H^-1
=
e_H
```

Поэтому:

```text
ab^-1 ∈ Ker φ
```

### 5. Kernel всегда normal

Более того:

```text
Ker φ ◁ G
```

Возьмём:

```text
k ∈ Ker φ
x ∈ G
```

Тогда:

```text
φ(xkx^-1)
=
φ(x)φ(k)φ(x)^-1
```

Но:

```text
φ(k) = e_H
```

поэтому:

```text
φ(xkx^-1)
=
φ(x)e_Hφ(x)^-1
=
e_H
```

Следовательно:

```text
xkx^-1 ∈ Ker φ
```

Значит kernel normal. Это один из главных источников normal subgroups:

> kernels of homomorphisms are always normal.

## Когда два элемента имеют одинаковый image

Для homomorphism:

```text
φ(a) = φ(b)
```

тогда и только тогда, когда:

```text
aKer φ = bKer φ
```

То есть два элемента переходят в одинаковый результат ровно тогда, когда лежат в одном coset kernel.

### Почему

Начнём с:

```text
φ(a) = φ(b)
```

Тогда:

```text
φ(a)^-1φ(b) = e_H
```

Следовательно:

```text
φ(a^-1b) = e_H
```

Значит:

```text
a^-1b ∈ Ker φ
```

А это означает:

```text
aKer φ = bKer φ
```

## Обратный образ элемента — это coset ядра

Пусть:

```text
φ: G → H
```

— гомоморфизм групп.

Его ядро состоит из всех элементов исходной группы, которые переходят в единичный элемент целевой группы:

```text
Ker φ = {k ∈ G | φ(k) = e_H}
```

Теперь возьмём элемент:

```text
y ∈ Im φ
```

и предположим, что:

```text
φ(g) = y
```

для некоторого `g ∈ G`.

Тогда все элементы исходной группы, которые также переходят в `y`, образуют coset ядра:

```text
φ⁻¹(y) = g Ker φ
```

Здесь:

```text
φ⁻¹(y)
```

обозначает не применение обратной функции, а **обратный образ** элемента `y`:

```text
φ⁻¹(y) = {x ∈ G | φ(x) = y}
```

Обратная функция у `φ` может вообще не существовать, если гомоморфизм не является взаимно однозначным. Но обратный образ элемента определить можно всегда.

### Почему получается именно coset ядра

Предположим, что элементы `g` и `x` имеют один и тот же образ:

```text
φ(g) = φ(x) = y
```

Тогда:

```text
φ(g⁻¹x)
= φ(g)⁻¹φ(x)
= y⁻¹y
= e_H
```

Следовательно:

```text
g⁻¹x ∈ Ker φ
```

Обозначим:

```text
k = g⁻¹x
```

Тогда:

```text
x = gk
```

где `k ∈ Ker φ`. Значит:

```text
x ∈ g Ker φ
```

Иными словами, любой элемент, который переходит в `y`, получается из `g` умножением на элемент ядра.

Верно и обратное. Если:

```text
x = gk
```

где `k ∈ Ker φ`, то:

```text
φ(x)
= φ(gk)
= φ(g)φ(k)
= y · e_H
= y
```

Поэтому все элементы coset `g Ker φ` имеют один и тот же образ `y`.

Итак:

```text
φ⁻¹(y) = g Ker φ
```

Главная идея состоит в том, что элементы ядра не влияют на результат гомоморфизма:

```text
φ(gk) = φ(g)
```

для любого:

```text
k ∈ Ker φ
```

Поэтому гомоморфизм не различает элементы одного coset ядра.

### Пример

Рассмотрим гомоморфизм:

```text
φ: Z → Z₄
φ(n) = n mod 4
```

Его ядро состоит из всех чисел, кратных `4`:

```text
Ker φ = 4Z
```

Обратный образ элемента `1 ∈ Z₄`:

```text
φ⁻¹(1)
= {..., -7, -3, 1, 5, 9, ...}
```

Это coset ядра:

```text
φ⁻¹(1) = 1 + 4Z
```

Все эти числа отличаются друг от друга на элемент ядра и поэтому имеют одинаковый остаток modulo `4`.

## Размер ядра и количество склеенных элементов

Пусть:

```text
φ: G → H
```

— гомоморфизм с конечным ядром, причём:

```text
|Ker φ| = n
```

Тогда каждый элемент образа гомоморфизма имеет ровно `n` прообразов.

Иными словами, отображение:

```text
φ: G → Im φ
```

является `n-to-1`: каждому элементу образа соответствуют ровно `n` элементов исходной группы.

Почему это так?

Если:

```text
φ(g) = y
```

то полный обратный образ элемента `y` равен coset ядра:

```text
φ⁻¹(y) = g Ker φ
```

Каждый coset содержит столько же элементов, сколько сама подгруппа `Ker φ`. Следовательно:

```text
|φ⁻¹(y)| = |g Ker φ| = |Ker φ| = n
```

То есть гомоморфизм склеивает элементы исходной группы группами по `n` штук.

Важно, что это утверждение относится только к элементам:

```text
y ∈ Im φ
```

Если элемент целевой группы не входит в образ гомоморфизма, его обратный образ пуст.

### Пример

Рассмотрим гомоморфизм аддитивных групп:

```text
φ: Z₁₂ → Z₄
φ(x) = x mod 4
```

Его ядро состоит из элементов `Z₁₂`, которые переходят в `0 ∈ Z₄`:

```text
Ker φ = {0, 4, 8}
```

Поэтому:

```text
|Ker φ| = 3
```

Посмотрим на обратные образы элементов `Z₄`:

```text
φ⁻¹(0) = {0, 4, 8}
φ⁻¹(1) = {1, 5, 9}
φ⁻¹(2) = {2, 6, 10}
φ⁻¹(3) = {3, 7, 11}
```

Каждый элемент `Z₄` имеет ровно три прообраза.

Например, числа:

```text
1, 5, 9
```

склеиваются в один элемент `1 ∈ Z₄`, потому что отличаются друг от друга на элементы ядра:

```text
5 = 1 + 4
9 = 1 + 8
```

И действительно:

```text
φ(1) = φ(5) = φ(9) = 1
```

Поэтому этот гомоморфизм является отображением `3-to-1` на свой образ.

В данном случае гомоморфизм сюръективен, поэтому:

```text
Im φ = Z₄
```

и каждый элемент всей целевой группы имеет ровно три прообраза.

## Числовой пример: `φ(x) = 3x` в `Z12`

Рассмотрим:

```text
φ : Z12 -> Z12
φ(x) = 3x mod 12
```

### Проверяем homomorphism

Операция в `Z12` — addition modulo `12`.

```text
φ(a + b)
=
3(a + b)
=
3a + 3b
```

Следовательно:

```text
φ(a + b) = φ(a) + φ(b)
```

Mapping является homomorphism.

### Считаем значения

```text
φ(0)  = 0
φ(1)  = 3
φ(2)  = 6
φ(3)  = 9
φ(4)  = 0
φ(5)  = 3
φ(6)  = 6
φ(7)  = 9
φ(8)  = 0
φ(9)  = 3
φ(10) = 6
φ(11) = 9
```

Image:

```text
φ(Z12) = {0, 3, 6, 9}
```

### Kernel

Ищем элементы, переходящие в additive identity `0`:

```text
3x ≡ 0 (mod 12)
```

Решения:

```text
x = 0, 4, 8
```

Поэтому:

```text
Ker φ = {0, 4, 8}
```

Kernel содержит `3` элемента. Следовательно, каждый элемент image имеет ровно `3` preimages.

### Cosets kernel

```text
K = Ker φ = {0, 4, 8}
```

Distinct cosets:

```text
0 + K = {0, 4, 8}
1 + K = {1, 5, 9}
2 + K = {2, 6, 10}
3 + K = {3, 7, 11}
```

Все элементы одного coset имеют одинаковый image:

```text
{0, 4, 8}   -> 0
{1, 5, 9}   -> 3
{2, 6, 10}  -> 6
{3, 7, 11}  -> 9
```

Например:

```text
φ^-1(6) = {2, 6, 10}
```

И действительно:

```text
φ^-1(6) = 2 + Ker φ
```

### Что здесь делает kernel

Mapping склеивает `Z12` в четыре блока:

```text
12 элементов
     ↓
4 cosets по 3 элемента
     ↓
4 элемента image
```

Kernel — это блок, который переходит в identity. Остальные preimages являются его сдвигами.

## Image подгруппы является подгруппой

Если:

```text
S ≤ G
```

то:

```text
φ(S)
```

является подгруппой `H`.

Здесь:

```text
φ(S) = {φ(s) | s ∈ S}
```

Например, в предыдущем mapping возьмём:

```text
S = <2> = {0, 2, 4, 6, 8, 10}
```

Тогда:

```text
φ(S) = {0, 6}
```

Это подгруппа `Z12`.

## Cyclic structure сохраняется в image

Если `S` cyclic:

```text
S = <a>
```

то:

```text
φ(S) = <φ(a)>
```

То есть image cyclic group также cyclic. Почему?

Каждый элемент `S` имеет вид:

```text
a^k
```

Поэтому его image:

```text
φ(a^k) = φ(a)^k
```

Следовательно, весь image порождается одним элементом `φ(a)`.

## Abelian structure сохраняется в image

Если `S` Abelian, то `φ(S)` тоже Abelian. Пусть:

```text
x = φ(a)
y = φ(b)
```

где:

```text
a, b ∈ S
```

Так как `S` Abelian:

```text
ab = ba
```

Следовательно:

```text
xy
=
φ(a)φ(b)
=
φ(ab)
=
φ(ba)
=
φ(b)φ(a)
=
yx
```

## Normal subgroup переходит в normal subgroup image

Если:

```text
N ◁ G
```

то:

```text
φ(N) ◁ φ(G)
```

Важно:

```text
φ(N)
```

гарантированно normal именно в `φ(G)`, а не обязательно во всём codomain `H`, если mapping не onto.

## Preimage подгруппы

Если:

```text
L ≤ H
```

то preimage:

```text
φ^-1(L)
=
{g ∈ G | φ(g) ∈ L}
```

является подгруппой `G`.

Если `L` normal in `H`, то:

```text
φ^-1(L) ◁ G
```

Kernel является частным случаем:

```text
Ker φ = φ^-1({e_H})
```

А trivial subgroup:

```text
{e_H}
```

всегда normal. Поэтому kernel всегда normal.

## Когда homomorphism становится isomorphism

Homomorphism one-to-one тогда и только тогда, когда:

```text
Ker φ = {e_G}
```

Почему?

Если два элемента имеют одинаковый image:

```text
φ(a) = φ(b)
```

то:

```text
aKer φ = bKer φ
```

Если kernel состоит только из identity, получаем:

```text
a = b
```

Следовательно, mapping injective.

Если homomorphism одновременно:

* onto;
* имеет trivial kernel;

то он является isomorphism.

## Homomorphisms из cyclic groups

Homomorphism из cyclic group полностью определяется тем, куда переходит generator.

Пусть:

```text
G = <a>
```

и мы выбрали:

```text
φ(a) = b
```

Тогда для любого элемента:

```text
a^k ∈ G
```

обязательно:

```text
φ(a^k) = b^k
```

Других вариантов нет. Но `b` нельзя выбирать совсем произвольно: он должен удовлетворять relations исходной группы.

## Пример: homomorphisms из `Z12` в `Z30`

Рассмотрим additive groups:

```text
φ : Z12 -> Z30
```

Группа `Z12` порождается элементом `1`.

Поэтому homomorphism полностью определяется значением:

```text
φ(1) = a
```

Тогда:

```text
φ(x) = xa mod 30
```

Но в `Z12`:

```text
12 · 1 = 0
```

Homomorphism обязан сохранить это relation:

```text
φ(12 · 1) = φ(0)
```

Значит:

```text
12φ(1) = 0 in Z30
```

То есть:

```text
12a ≡ 0 (mod 30)
```

Решения:

```text
a = 0, 5, 10, 15, 20, 25
```

Следовательно, существует шесть homomorphisms:

```text
φ_0(x)  = 0
φ_5(x)  = 5x mod 30
φ_10(x) = 10x mod 30
φ_15(x) = 15x mod 30
φ_20(x) = 20x mod 30
φ_25(x) = 25x mod 30
```

Количество получилось равным:

```text
gcd(12, 30) = 6
```

В общем случае число homomorphisms:

```text
Z_n -> Z_m
```

равно:

```text
gcd(n, m)
```

## Зачем это нужно

Homomorphisms позволяют изучать сложную группу через более простую.

Они показывают:

* какая часть структуры сохраняется;
* какая часть склеивается;
* что именно теряется;
* какие normal subgroups возникают;
* как исходная группа связана со своим image;
* почему quotient groups появляются естественным образом.

Kernel описывает потерянную информацию. Image описывает сохранившуюся информацию. А cosets kernel показывают, какие элементы mapping перестал различать.

## Связь с factor groups

В примере:

```text
φ : Z12 -> Z12
φ(x) = 3x mod 12
```

мы получили:

```text
Ker φ = {0, 4, 8}
```

и:

```text
φ(Z12) = {0, 3, 6, 9}
```

Cosets kernel:

```text
{0,4,8}
{1,5,9}
{2,6,10}
{3,7,11}
```

соответствуют четырём элементам image.

Поэтому возникает связь:

```text
Z12 / Ker φ ≅ φ(Z12)
```

Это не случайность. Дальше она будет сформулирована как **First Isomorphism Theorem**:

```text
G / Ker φ ≅ φ(G)
```

То есть quotient group по kernel всегда изоморфна image homomorphism.

## First Isomorphism Theorem

Мы уже увидели две вещи. Во-первых, homomorphism:

```text
φ : G -> H
```

может отправлять несколько разных элементов `G` в один и тот же элемент `H`.

Во-вторых, элементы, имеющие одинаковый image, лежат в одном coset kernel:

```text
φ(a) = φ(b)
```

тогда и только тогда, когда:

```text
a Ker φ = b Ker φ
```

Это означает, что homomorphism склеивает элементы `G` не случайным образом.

Он склеивает вместе ровно элементы одного coset:

```text
g Ker φ
```

После такого склеивания получается quotient group:

```text
G / Ker φ
```

А First Isomorphism Theorem утверждает, что эта quotient group имеет ту же структуру, что и image homomorphism.

## Формулировка теоремы

Пусть:

```text
φ : G -> H
```

— group homomorphism.

Тогда:

```text
G / Ker φ ≅ φ(G)
```

Здесь:

```text
Ker φ
```

— элементы `G`, переходящие в identity, а:

```text
φ(G)
```

— image, то есть часть `H`, в которую mapping действительно попадает.

Isomorphism задаётся правилом:

```text
g Ker φ -> φ(g)
```

То есть каждому coset kernel мы сопоставляем общий image всех его элементов.

### Что теорема говорит человеческим языком

Homomorphism делает с группой `G` две вещи:

1. склеивает элементы, которые отличаются на элемент kernel;
2. оставляет в результате только image.

First Isomorphism Theorem говорит:

> если сначала самостоятельно склеить элементы `G` по cosets kernel, то получится группа, изоморфная image homomorphism.

Схематично:

```text
G
│
│ склеиваем каждый coset Ker φ
▼
G / Ker φ
│
│ isomorphism
▼
φ(G)
```

Или ещё короче:

```text
исходная группа / потерянная информация
=
сохранившаяся информация
```

## Пример: `φ(x) = 3x` в `Z12`

Рассмотрим homomorphism:

```text
φ : Z12 -> Z12
```

заданный правилом:

```text
φ(x) = 3x mod 12
```

Мы уже вычисляли его значения:

```text
φ(0)  = 0
φ(1)  = 3
φ(2)  = 6
φ(3)  = 9
φ(4)  = 0
φ(5)  = 3
φ(6)  = 6
φ(7)  = 9
φ(8)  = 0
φ(9)  = 3
φ(10) = 6
φ(11) = 9
```

Image:

```text
φ(Z12) = {0, 3, 6, 9}
```

Kernel:

```text
Ker φ = {0, 4, 8}
```

Обозначим:

```text
K = Ker φ
```

### Cosets kernel

Distinct cosets подгруппы `K`:

```text
0 + K = {0, 4, 8}
1 + K = {1, 5, 9}
2 + K = {2, 6, 10}
3 + K = {3, 7, 11}
```

Именно эти четыре cosets являются элементами quotient group:

```text
Z12 / K
```

### Все элементы одного coset имеют одинаковый image

Для первого coset:

```text
φ(0) = φ(4) = φ(8) = 0
```

Для второго:

```text
φ(1) = φ(5) = φ(9) = 3
```

Для третьего:

```text
φ(2) = φ(6) = φ(10) = 6
```

Для четвёртого:

```text
φ(3) = φ(7) = φ(11) = 9
```

Поэтому можно определить mapping:

```text
Ψ : Z12 / K -> φ(Z12)
```

так:

```text
Ψ(0 + K) = 0
Ψ(1 + K) = 3
Ψ(2 + K) = 6
Ψ(3 + K) = 9
```

Или общей формулой:

```text
Ψ(x + K) = φ(x)
```

Получаем соответствие:

```text
{0, 4, 8}   -> 0
{1, 5, 9}   -> 3
{2, 6, 10}  -> 6
{3, 7, 11}  -> 9
```

## Почему mapping между quotient group и image корректен

Один coset можно записать через разных representatives.

Например:

```text
1 + K = 5 + K = 9 + K
```

Поэтому нужно убедиться, что формула:

```text
Ψ(x + K) = φ(x)
```

не зависит от выбранного representative.

В нашем примере:

```text
φ(1) = φ(5) = φ(9) = 3
```

Поэтому независимо от того, напишем мы:

```text
1 + K
```

```text
5 + K
```

или:

```text
9 + K
```

результат будет один `3`.

Это верно в общем случае, потому что:

```text
a Ker φ = b Ker φ
```

означает:

```text
a^-1b ∈ Ker φ
```

Следовательно:

```text
φ(a^-1b) = e
```

Но:

```text
φ(a^-1b) = φ(a)^-1φ(b)
```

Значит:

```text
φ(a)^-1φ(b) = e
```

и поэтому:

```text
φ(a) = φ(b)
```

## Проверяем свойства isomorphism

Чтобы mapping:

```text
Ψ(g Ker φ) = φ(g)
```

был isomorphism, нужно проверить четыре вещи.

### 1. Mapping well-defined

Если два representatives задают один coset:

```text
a Ker φ = b Ker φ
```

то:

```text
φ(a) = φ(b)
```

Поэтому один элемент quotient group не может получить два разных image.

### 2. Mapping onto

Codomain mapping `Ψ` — это:

```text
φ(G)
```

То есть множество всех значений вида:

```text
φ(g)
```

Поэтому каждый элемент image автоматически является образом некоторого coset:

```text
g Ker φ
```

Следовательно, `Ψ` onto.

### 3. Mapping one-to-one

Допустим:

```text
Ψ(a Ker φ) = Ψ(b Ker φ)
```

Тогда:

```text
φ(a) = φ(b)
```

А мы уже знаем, что это означает:

```text
a Ker φ = b Ker φ
```

Следовательно, разные cosets не переходят в один элемент.

Mapping one-to-one.

### 4. Операция сохраняется

В quotient group:

```text
(a Ker φ)(b Ker φ) = ab Ker φ
```

Применяем `Ψ`:

```text
Ψ(ab Ker φ) = φ(ab)
```

Так как `φ` — homomorphism:

```text
φ(ab) = φ(a)φ(b)
```

А это:

```text
Ψ(a Ker φ)Ψ(b Ker φ)
```

Поэтому:

```text
Ψ((a Ker φ)(b Ker φ))
=
Ψ(a Ker φ)Ψ(b Ker φ)
```

Операция сохраняется.

Следовательно:

```text
G / Ker φ ≅ φ(G)
```

## Операция в числовом примере

Вернёмся к:

```text
φ(x) = 3x mod 12
```

Возьмём в quotient group:

```text
(2 + K) + (3 + K)
```

Складываем representatives:

```text
2 + 3 = 5
```

Поэтому:

```text
(2 + K) + (3 + K) = 5 + K
```

Но:

```text
5 + K = 1 + K
```

Следовательно:

```text
(2 + K) + (3 + K) = 1 + K
```

Теперь посмотрим на соответствующие элементы image:

```text
Ψ(2 + K) = 6
```

```text
Ψ(3 + K) = 9
```

Складываем modulo `12`:

```text
6 + 9 = 15 ≡ 3 (mod 12)
```

А:

```text
Ψ(1 + K) = 3
```

Получили одинаковый результат.

Именно это означает, что mapping сохраняет операцию.

## Почему quotient group и image имеют одинаковый размер

Для finite group:

```text
|G / Ker φ|
=
|G| / |Ker φ|
```

Но по First Isomorphism Theorem:

```text
G / Ker φ ≅ φ(G)
```

Поэтому:

```text
|φ(G)|
=
|G| / |Ker φ|
```

Или:

```text
|G|
=
|Ker φ| · |φ(G)|
```

Эта формула очень полезна.

Она говорит, что размер исходной группы состоит из:

```text
размер одного склеиваемого блока
×
количество получившихся результатов
```

То есть:

```text
|Ker φ|
```

— количество элементов в каждом preimage, а:

```text
|φ(G)|
```

— количество разных значений mapping.

### В примере с `Z12`

```text
|Z12| = 12
|Ker φ| = 3
|φ(Z12)| = 4
```

И действительно:

```text
12 = 3 · 4
```

Mapping является `3-to-1` на свой image:

```text
{0,4,8}   -> 0
{1,5,9}   -> 3
{2,6,10}  -> 6
{3,7,11}  -> 9
```

## Следствие для порядков групп

Если `G` finite, то:

```text
|φ(G)| divides |G|
```

потому что:

```text
|G| = |Ker φ| · |φ(G)|
```

Кроме того, если codomain `H` тоже finite, то:

```text
|φ(G)| divides |H|
```

потому что image:

```text
φ(G)
```

является подгруппой `H`, а порядок подгруппы делит порядок группы по Lagrange's Theorem.

## Пример: `Z / nZ ≅ Z_n`

Рассмотрим гомоморфизм аддитивных групп:

```text
φ: Z → Z_n
```

заданный формулой:

```text
φ(x) = x mod n
```

Он отправляет каждое целое число в его остаток при делении на `n`.

Например, при `n = 5`:

```text
φ(0) = 0
φ(1) = 1
φ(5) = 0
φ(6) = 1
φ(11) = 1
φ(-1) = 4
```

Этот гомоморфизм сохраняет сложение:

```text
φ(a + b) = φ(a) + φ(b)
```

где сложение справа выполняется modulo `n`.

Например:

```text
φ(8 + 9)
= φ(17)
= 2
```

и одновременно:

```text
φ(8) + φ(9)
= 3 + 4
= 7
≡ 2 (mod 5)
```

Поэтому `φ` действительно является гомоморфизмом.

### Что здесь склеивает гомоморфизм

Разные целые числа могут иметь одинаковый остаток modulo `n`.

Например, при `n = 5`:

```text
1, 6, 11, 16, ...
```

все переходят в `1 ∈ Z_5`.

Также:

```text
2, 7, 12, 17, ...
```

все переходят в `2 ∈ Z_5`.

Гомоморфизм перестаёт различать числа, которые отличаются на число, кратное `n`:

```text
φ(a) = φ(b)
```

тогда и только тогда, когда:

```text
a ≡ b (mod n)
```

то есть:

```text
a - b ∈ nZ
```

Именно эти группы склеенных чисел становятся элементами quotient group.

### Kernel

Kernel состоит из всех целых чисел, которые переходят в единичный элемент аддитивной группы `Z_n`, то есть в `0`:

```text
Ker φ = {x ∈ Z | φ(x) = 0}
```

Остаток числа равен нулю тогда и только тогда, когда число делится на `n`. Поэтому:

```text
Ker φ = nZ
```

где:

```text
nZ = {..., -2n, -n, 0, n, 2n, ...}
```

Например, при `n = 5`:

```text
Ker φ = 5Z
       = {..., -10, -5, 0, 5, 10, ...}
```

### Что представляет собой `Z / nZ`

Quotient group:

```text
Z / nZ
```

состоит не из отдельных целых чисел, а из cosets подгруппы `nZ`:

```text
a + nZ
```

Каждый такой coset содержит все целые числа, имеющие один и тот же остаток modulo `n`.

Например, при `n = 5`:

```text
0 + 5Z = {..., -10, -5, 0, 5, 10, ...}
1 + 5Z = {..., -9, -4, 1, 6, 11, ...}
2 + 5Z = {..., -8, -3, 2, 7, 12, ...}
3 + 5Z = {..., -7, -2, 3, 8, 13, ...}
4 + 5Z = {..., -6, -1, 4, 9, 14, ...}
```

Других cosets нет. Например:

```text
6 + 5Z = 1 + 5Z
```

потому что:

```text
6 - 1 = 5 ∈ 5Z
```

Поэтому:

```text
Z / 5Z
```

содержит ровно пять элементов:

```text
0 + 5Z
1 + 5Z
2 + 5Z
3 + 5Z
4 + 5Z
```

Они естественным образом соответствуют остаткам:

```text
0, 1, 2, 3, 4
```

в группе `Z_5`.

### Почему операции тоже совпадают

В quotient group сложение определяется так:

```text
(a + nZ) + (b + nZ) = (a + b) + nZ
```

Например, в `Z / 5Z`:

```text
(3 + 5Z) + (4 + 5Z)
= 7 + 5Z
= 2 + 5Z
```

В `Z_5` происходит то же самое:

```text
3 + 4 ≡ 2 (mod 5)
```

То есть соответствие:

```text
a + nZ ↦ a mod n
```

сохраняет групповую операцию.

### Применение First Isomorphism Theorem

Образ гомоморфизма равен всей группе `Z_n`, потому что каждый остаток:

```text
0, 1, ..., n - 1
```

является образом некоторого целого числа:

```text
φ(Z) = Z_n
```

Мы также нашли kernel:

```text
Ker φ = nZ
```

По First Isomorphism Theorem:

```text
Z / Ker φ ≅ φ(Z)
```

Подставляем kernel и image:

```text
Z / nZ ≅ Z_n
```

Смысл этого изоморфизма заключается в следующем:

> Группа `Z_n` получается из `Z`, если склеить все целые числа, имеющие одинаковый остаток modulo `n`.

Например, все числа из coset:

```text
3 + nZ
```

становятся одним элементом:

```text
3 ∈ Z_n
```

Таким образом, обычная модульная арифметика — это не просто похожая конструкция. Она буквально является quotient group целых чисел по подгруппе чисел, кратных `n`, а First Isomorphism Theorem объясняет, почему эти две группы изоморфны.

## Natural mapping

Для любой normal subgroup:

```text
N ◁ G
```

существует естественный homomorphism:

```text
π : G -> G / N
```

заданный правилом:

```text
π(g) = gN
```

Он называется:

```text
natural mapping
```

или:

```text
canonical projection
```

Mapping просто отправляет каждый элемент `G` в coset, которому он принадлежит.

### Проверяем сохранение операции

```text
π(ab) = abN
```

А:

```text
π(a)π(b)
=
(aN)(bN)
=
abN
```

Следовательно:

```text
π(ab) = π(a)π(b)
```

Значит `π` — homomorphism.

### Kernel natural mapping

Ищем элементы, которые переходят в identity quotient group.

Identity в:

```text
G / N
```

— это coset:

```text
N
```

Поэтому:

```text
Ker π
=
{g ∈ G | gN = N}
```

Но:

```text
gN = N
```

тогда и только тогда, когда:

```text
g ∈ N
```

Следовательно:

```text
Ker π = N
```

## Каждая normal subgroup является kernel

Ранее мы доказали:

> kernel любого homomorphism является normal subgroup.

Теперь получили обратное утверждение:

> каждая normal subgroup является kernel некоторого homomorphism.

Пусть:

```text
N ◁ G
```

Тогда natural mapping:

```text
π : G -> G / N
```

```text
π(g) = gN
```

имеет kernel:

```text
Ker π = N
```

Следовательно:

```text
normal subgroups
=
possible kernels of homomorphisms
```

Это объясняет, почему normal subgroups настолько важны. Они не просто подгруппы, у которых совпадают left и right cosets. Они описывают именно ту часть группы, которую некоторый homomorphism может превратить в identity.

## Замкнувшийся круг

Теперь все понятия связываются вместе.

### Homomorphism

```text
φ : G -> H
```

сохраняет group operation.

### Kernel

```text
Ker φ
```

показывает, какие элементы склеиваются с identity.

### Cosets kernel

```text
g Ker φ
```

— наборы элементов, имеющих одинаковый image.

### Quotient group

```text
G / Ker φ
```

получается после склеивания каждого такого набора в один элемент.

### Image

```text
φ(G)
```

— структура, которая остаётся после mapping.

### First Isomorphism Theorem

```text
G / Ker φ ≅ φ(G)
```

То есть quotient group по потерянной части изоморфна сохранившейся части.

## Homomorphism раскладывается на два шага

Mapping:

```text
φ : G -> φ(G)
```

можно понимать как композицию двух mappings.

Сначала natural projection:

```text
π : G -> G / Ker φ
```

```text
π(g) = g Ker φ
```

Она склеивает элементы одного coset.

Затем isomorphism:

```text
Ψ : G / Ker φ -> φ(G)
```

```text
Ψ(g Ker φ) = φ(g)
```

Он просто переименовывает получившиеся cosets в элементы image.

Поэтому:

```text
φ = Ψ ∘ π
```

Схематично:

```text
          φ
G -----------------> φ(G)
 \                   ▲
  \ π                │ Ψ
   ▼                 │
    G / Ker φ --------
```

Какой бы маршрут мы ни выбрали, результат один:

```text
Ψ(π(g)) = φ(g)
```

Такая схема называется **commutative diagram / коммутативной диаграммой**.

## Почему это полезно

First Isomorphism Theorem позволяет не изучать homomorphism как непонятный набор стрелок между элементами.

Вместо этого можно:

1. найти kernel;
2. построить quotient group;
3. сразу понять структуру image.

Например, если:

```text
|G| = 60
```

и:

```text
|Ker φ| = 12
```

то:

```text
|φ(G)| = 60 / 12 = 5
```

Следовательно, image имеет порядок `5`. А любая группа prime order cyclic, поэтому:

```text
φ(G) ≅ Z5
```

Мы узнали структуру image, практически не вычисляя сами значения mapping.

## Важное различие: image и homomorphism

First Isomorphism Theorem описывает возможную структуру image. Но разные homomorphisms могут иметь один и тот же image.

Например, mappings:

```text
φ1 : Z -> Z5
φ1(x) = x mod 5
```

и:

```text
φ2 : Z -> Z5
φ2(x) = 2x mod 5
```

различны.

Например:

```text
φ1(1) = 1
```

а:

```text
φ2(1) = 2
```

Но обе имеют image:

```text
Z5
```

Поэтому количество homomorphisms и количество различных homomorphic images — не одно и то же.

## Главная идея

> Homomorphism склеивает элементы одного coset kernel. После этого склеивания quotient group имеет ровно ту же структуру, что и image homomorphism.