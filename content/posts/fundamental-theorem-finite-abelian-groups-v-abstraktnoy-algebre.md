...
Более того, такое разложение по существу единственно.
...
Теорема относится к группам, которые одновременно:
...
В finite Abelian groups происходит похожая история.

Часть группы, связанная с одним prime `p`, можно отделить от частей, связанных с другими primes.
...
Теорема утверждает не просто существование какого-то разложения.

Она говорит, что набор cyclic prime-power components определяется группой uniquely, с точностью до:
...
Знать порядок группы недостаточно.

Например, все следующие группы имеют порядок `16`:
...
Для произвольной finite group такой алгоритм может сломаться.

Но для finite Abelian p-groups существует важный результат:
...
# Corollary: подгруппы всех допустимых порядков
...
# Идея доказательства фундаментальной теоремы

Полное доказательство довольно длинное, но его структура понятна.

Оно состоит из двух больших частей:

1. existence / существование разложения;
2. uniqueness / уникальность разложения.

---

# Шаг 1: разделяем разные primes
...
* часть группы, элементы которой имеют `p`-power order;
* часть группы, порядки элементов которой делят `m`.

Так как группа Abelian, эти части образуют normal subgroups и дают internal direct product:
...
Так можно разделить группу на её prime components.

---

# Шаг 2: разбираем Abelian p-group

Теперь рассматриваем группу порядка:

```text
p^n
```

Выбираем элемент `a` максимального порядка.

Ключевая lemma говорит, что существует subgroup `K`, для которой:
...
К `K` снова применяем тот же аргумент.

Так постепенно получаем:
...
Каждый cyclic factor имеет порядок:
...
# Шаг 3: доказываем uniqueness

Допустим, группа получила два разложения:
...
где все компоненты cyclic prime-power groups.
...
и после подходящей перестановки:
...
То есть два действительно разных набора prime-power cyclic factors описывать одну и ту же группу не могут.
...
Она не утверждает, что любая finite group является direct product cyclic groups.

Например, non-Abelian groups требуют совершенно других методов классификации.
...
Каждая finite Abelian group изоморфна direct product cyclic groups prime-power order:
...
Набор prime-power cyclic components определяется группой uniquely с точностью до перестановки компонентов.

---

## Группы порядка `p^k`

Isomorphism classes соответствуют partitions числа `k`.
...
то отдельно классифицируем components порядков:

```text
p1^a1, p2^a2, ..., pk^ak
```

а затем берём все возможные direct products.

---

## Subgroups

Если `G` — finite Abelian group и:

```text
m divides |G|
```

то в `G` существует subgroup порядка `m`.
