...
Берём обычный квадрат. Предположим, что мы вводим для него несколько операций. По нашим условным правилам его можно:

- поворачивать на 0, 90, 180, 270 и так далее градусов (rotate);
- отражать по вертикали, горизонтали, диагонали (reflect);
- делать несколько таких операций подряд.

Нас интересует не сам путь движения, а итоговый эффект (net effect), то есть как этот квадрат будет выглядеть в итоге.
...
Эти движения можно обозначить так:
...
Операция в `D4` — это **композиция движений** (composition of motions). То есть можно сделать одно движение, потом второе, и получить итоговое движение.
