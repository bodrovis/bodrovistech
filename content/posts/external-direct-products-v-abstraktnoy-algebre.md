...
Как ни странно, Китайская теорема об остатках может использоваться для взлома (в частности, для прочтения шифрованного сообщения). Идея такая. В RSA encryption обычно выглядит так:
...
и отправляет один и тот же message нескольким людям без padding. Хотя `e` входит в открытый ключ, оно не обязано быть уникальным. На практике многие пользователи используют одно и то же значение `e`, а различаются их ключи за счёт разных модулей `n`.
